package BackgoundStepDefinition;

import io.cucumber.java.en.And;
import io.cucumber.java.en.Given;
import io.cucumber.java.en.Then;
import io.cucumber.java.en.When;

public class BackgoundStepDefinition {
	@Given("User access the saucedemo app login page")
	public void user_access_the_saucedemo_app_login_page() {


	}

	@When("User enters valid creds")
	public void user_enters_valid_creds() {


	}

	@And("click on breadcrumb icon")
	public void click_on_breadcrumb_icon() {


	}

	@Then("user should be able to see the menu items")
	public void user_should_be_able_to_see_the_menu_items() {


	}

	@Given("Users access the saucedemo app login page")
	public void users_access_the_saucedemo_app_login_page() {


	}

	@When("Users enters the valid creds")
	public void users_enters_the_valid_creds() {


	}

	@And("clicks on Add to button")
	public void clicks_on_add_to_button() {


	}

	@Then("Item should be added to the Cart")
	public void item_should_be_added_to_the_cart() {


	}

	@Given("User is accessing saucedemo login page")
	public void user_is_accessing_saucedemo_login_page() {


	}

	@When("User is enters valid username and password")
	public void user_is_enters_valid_username_and_password() {


	}

	@Then("User should be able to nivgate to home page")
	public void user_should_be_able_to_nivgate_to_home_page() {


	}
}
